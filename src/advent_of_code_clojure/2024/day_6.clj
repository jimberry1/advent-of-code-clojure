...
(defn prepare-input [input]
  (utils/format-input input :line-re-split #"" :input-xform utils/indexed-grid-map))
...
(defn walk-grid [grid-map start-item]
  (loop [steps (list start-item) cur-item start-item direction :up]
    (let [next-item (utils/get-value-in-direction grid-map direction cur-item)]
      (cond
        (nil? next-item) steps
...
(defn part-1 [input]
  (let [grid-map (prepare-input input)
        start-item (utils/find-grid-item grid-map "^")
...
(defn part-2 [input]
  (let [grid-map (prepare-input input)
    ...
    (->> steps
         ...
         (map (partial utils/replace-grid-item grid-map "#"))
