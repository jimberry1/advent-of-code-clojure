...
(defn x->xmas-counter [grid-map directions x-grid-item]
  (reduce (fn [acc direction]
            (let [grid-items-in-direction (utils/walk-grid grid-map direction x-grid-item :steps 3)
...
(defn part-1 [puzzle-input]
  (let [grid-map (prepare-input puzzle-input)
        x-locations (->> grid-map utils/grid-map->grid-coll (filter #(= "X" (:val %))))
        allowed-directions (keys utils/all-directions)]
